...
alter table `distance groups` rename column  `ï»¿%Distance Group ID` to `distance_id`;
...
VIEW `distance` AS
    SELECT 
        `distance groups`.`distance_id` AS `distance_id`,
        `distance groups`.`distance_interval` AS `distance_interval`,
        COUNT(`maindata`.`airline_id`) AS `count(airline_id)`
    FROM
        (`distance groups`
        LEFT JOIN `maindata` ON ((`distance groups`.`distance_id` = `maindata`.`distance_id`)))
    GROUP BY `distance groups`.`distance_id` , `distance groups`.`distance_interval`;
